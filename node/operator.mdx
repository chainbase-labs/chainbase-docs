...
#  run task 
docker run -itd -p 8080:8080 --mount type=bind,source=/path/to/operator.yaml,target=/opt/operator.yaml   --mount type=bind,source=/path/to/ecdsa.key.json,target=/opt/ecdsa.key.json   chainbase-avs:latest
```